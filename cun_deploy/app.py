...
.sub-fechas { font-size: 0.88rem; color: #64748b; }
.sub-fechas strong { color: #1e293b; font-weight: 500; }
.dias-restantes { margin-top: 0.4rem; font-size: 0.78rem; font-weight: 600; }
.dr-act  { color: #059669; }
...
def estado_sub(ini, fin):
    hoy = date.today()
    if ini is None or fin is None: return "desconocido"
...
def dias_info(ini, fin):
    ...
    if hoy > fin: return "finalizado", "Finalizado"
    dias = (fin - hoy).days
...
def estado_bloque(data):
    ...
    for sub in ["I", "II"]:
        rng = data.get(sub)
        if rng: estados.append(estado_sub(rng[0], rng[1]))
    if "activo" in estados: return "activo"
    if "proximo" in estados: return "proximo"
    return "finalizado"
...
def mostrar_actividades(actos):
    ...
    for a in actos:
        ...
        if a["ini"] <= hoy <= a["fin"]:
            css_row = "vigente"; dot = '<span style="color:#059669">●</span>'
        elif a["ini"] > hoy:
            css_row = "proxima"; dot = '<span style="color:#2563eb">●</span>'
        else:
            css_row = "done"; dot = '<span style="color:#cbd5e1">●</span>'
...
# ── TABS ──────────────────────────────────────────────────────────────────────
tab1, tab2, tab_be, tab_kun = st.tabs(["🔍 Buscar bloque", "📋 Todos los bloques", "🌐 Calendario BE", "📍 Días KUN"])

# ══ TAB 1 ════════════════════════════════════════════════════════════════════
with tab1:
    opciones = {d["label"]: c for c, d in CALENDARIOS.items() if not d.get("alias")}
    sel_label = st.selectbox("Selecciona un bloque", ["— elige un bloque —"] + list(opciones.keys()))
    ...
    for i, (sub_key, sub_label) in enumerate([("I","Bloque I"),("II","Bloque II")]):
        ...
        with cols[i]:
            if rng is None:
                st.markdown(f'<div class="sub-card done-sub"><div class="sub-titulo t-done">{sub_label}</div><div class="sub-fechas">No aplica</div></div>', unsafe_allow_html=True)
            else:
    ...
    for sub_key in ["I", "II"]:
        ...
        if clave in ACTIVIDADES_MAP:
            rng = data.get(sub_key)
            if rng and estado_sub(rng[0], rng[1]) in ("activo", "proximo"):
...
with tab2:
    st.markdown("<br>", unsafe_allow_html=True)
    filtro = st.multiselect("Filtrar por estado", ["🟢 Activo","🔵 Próximo","⬛ Finalizado"], default=["🟢 Activo","🔵 Próximo"])
    mapa_filtro = {"🟢 Activo":"activo","🔵 Próximo":"proximo","⬛ Finalizado":"finalizado"}
    estados_sel = [mapa_filtro[f] for f in filtro]
    for codigo, data in CALENDARIOS.items():
        if data.get("alias"): continue
        est_gral = estado_bloque(data)
        if estados_sel and est_gral not in estados_sel: continue
        ...
        <div class="bloque-card {card_cls[est_gral]}">
            <div class="bloque-nombre">{data['label']}
...
with tab_kun:
    ...
    for d in DIAS_KUN:
        ...
        if diff < 0:
            bg="#f8fafc"; borde="#e2e8f0"; txt="#94a3b8"
            badge_bg="#f1f5f9"; badge_col="#64748b"; em="⬛"; etiqueta="Finalizado"
        elif diff == 0:
            bg="#f0fdf4"; borde="#6ee7b7"; txt="#065f46"
            badge_bg="#d1fae5"; badge_col="#065f46"; em="🟢"; etiqueta="¡HOY!"
        elif diff == 1:
            bg="#fefce8"; borde="#fde68a"; txt="#92400e"
            badge_bg="#fef3c7"; badge_col="#92400e"; em="🟡"; etiqueta="¡MAÑANA!"
        else:
            bg="#eff6ff"; borde="#93c5fd"; txt="#1e40af"
            badge_bg="#dbeafe"; badge_col="#1e40af"; em="🔵"; etiqueta=f"En {diff} días"
        ...
        <div style="background:{bg};border:1px solid {borde};border-left:4px solid {borde};
            ...
            <div>
                ...
                <span style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:700;color:{txt};">
                    {em} {d['dia']} {d['fecha'].strftime('%d/%m/%Y')}
                </span>
            ...
            <div>
                <span style="background:{badge_bg};color:{badge_col};padding:0.3rem 0.8rem;
